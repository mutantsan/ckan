...
import ckanext.ckan_admin.table as table
import ckanext.ckan_admin.types as ap_types
from ckanext.ckan_admin.logic import schema as ap_schema
...
ckan_admin_user = Blueprint("ckan_admin_user", __name__, url_prefix="/ckan-admin")
ckan_admin_user.before_request(ckan_admin_before_request)
...
class UserTable(table.TableDefinition):
    def __init__(self):
        super().__init__(
            name="user",
            ajax_url=tk.url_for("ckan_admin_user.list", data=True),
            ...
            global_actions=[
                table.GlobalActionDefinition(
                    action="add_sysadmin",
                    label=tk._("Add sysadmin role to selected users"),
                ),
                table.GlobalActionDefinition(
                    action="remove_sysadmin",
                    label=tk._("Remove sysadmin role from selected users"),
                ),
                table.GlobalActionDefinition(
                    action="block", label=tk._("Block selected users")
                ),
                table.GlobalActionDefinition(
                    action="unblock", label=tk._("Unblock selected users")
                ),
            ],
        )
...
class UserAddView(MethodView):
    def get(
        ...
        return tk.render(
            "ckan_admin/config/user/create_form.html",
            extra_vars={
                "data": data or {},
                "errors": errors or {},
    ...
    def post(self) -> str | Response:
        context = types.Context(
            ...
            schema=ap_schema.ckan_admin_new_user_schema(),
        ...
        tk.h.flash_success(
            tk._(f"A new user account has been created for {link}"), allow_html=True
        )
        log.info("User '%s' has been created", user_dict["name"])

        return tk.redirect_to("ckan_admin_user.create")
